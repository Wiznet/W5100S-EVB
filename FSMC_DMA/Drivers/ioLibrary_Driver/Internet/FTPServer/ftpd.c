...
un_l2cval remote_ip_server;
uint16_t  remote_port_server;
un_l2cval local_ip_server;
uint16_t  local_port_server;
...
void ftpd_init(uint8_t * src_ip)
{
	...
	local_ip_server.cVal[0] = src_ip[0];
	local_ip_server.cVal[1] = src_ip[1];
	local_ip_server.cVal[2] = src_ip[2];
	local_ip_server.cVal[3] = src_ip[3];
	local_port_server = 35000;
    	...
    	case SOCK_ESTABLISHED :
...
#if defined(_FTP_DEBUG_)
   					printf("%d:FTPDataStart, port : %d\r\n",DATA_SOCK, local_port_server);
#endif
   					if((ret=socket(DATA_SOCK, Sn_MR_TCP, local_port_server, 0x0)) != DATA_SOCK)
   					{
#if defined(_FTP_DEBUG_)
   						printf("%d:socket() error:%ld\r\n", DATA_SOCK, ret);
#endif
   		...
   		case SOCK_INIT :
   			...
   			if(ftp.dsock_mode == PASSIVE_MODE){
   				...
   				if((ret = connect(DATA_SOCK, remote_ip_server.cVal, remote_port_server)) != SOCK_OK){
#if defined(_FTP_DEBUG_)
...
char proc_ftpd(char * buf)
{
	...
	switch (cmdp - commands)
	{
		...
		case PASV_CMD:
			slen = sprintf(sendbuf, "227 Entering Passive Mode (%d,%d,%d,%d,%d,%d)\r\n", local_ip_server.cVal[0], local_ip_server.cVal[1], local_ip_server.cVal[2], local_ip_server.cVal[3], local_port_server >> 8, local_port_server & 0x00ff);
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			disconnect(DATA_SOCK);
...
int pport(char * arg)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		remote_ip_server.cVal[i] = (uint8_t)atoi(tok);
		if (!tok)
		{
#if defined(_FTP_DEBUG_)
			printf("bad pport : %s\r\n", arg);
	...
	remote_port_server = 0;
	for (i = 0; i < 2; i++)
	{
		tok = strtok(NULL,",\r\n");
		remote_port_server <<= 8;
		remote_port_server += atoi(tok);
		if (!tok)
		{
#if defined(_FTP_DEBUG_)
			printf("bad pport : %s\r\n", arg);
...
#if defined(_FTP_DEBUG_)
	printf("ip : %d.%d.%d.%d, port : %d\r\n", remote_ip_server.cVal[0], remote_ip_server.cVal[1], remote_ip_server.cVal[2], remote_ip_server.cVal[3], remote_port_server);
#endif
